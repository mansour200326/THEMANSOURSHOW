  ...
  if (state.phase === "done") {
    ...
    return (
      <main className="flex min-h-dvh lg:h-dvh flex-col items-center justify-center gap-[3vmin] p-[4vmin] text-center pb-16 lg:pb-[1.6vmin]">
        <p className="t-label font-display uppercase text-moon-dim">{t("That's the game")}</p>
        <WinnerMoment>
          {ranked[0]?.name ?? t("Nobody")} wins
        </WinnerMoment>
  ...
  return (
    <main className="flex min-h-dvh lg:h-dvh flex-col gap-[2vmin] p-[2.5vmin] pb-16 lg:pb-[1.6vmin]">
      ...
      <div className="flex min-h-0 flex-1 flex-col items-center justify-center gap-[2.5vmin] px-[3vw] text-center">
        ...
        {state.phase === "answer" && (
          <>
            <p className="t-label font-display uppercase text-moon-dim">{t("Look at your phone")}</p>
            <h2 className="t-clue text-balance font-display uppercase tracking-wide text-moon">{t("Everyone has a question. One of you has a different one.")}</h2>
            <p className="max-w-3xl text-[clamp(1rem,1.8vw,1.8rem)] text-moon-dim">{t("Answer yours. Then work out who was answering something else.")}</p>
            ...
            <p className="font-display text-[clamp(0.9rem,1.5vw,1.5rem)] uppercase tracking-[0.2em] text-moon-dim">
              {waiting.length
                ? t("Waiting on {names}", { names: waiting.map((p) => p.name).join("، ") })
                : "Everyone's in"}
            </p>
          </>
        )}
        ...
        {state.phase === "vote" && (
          <>
            <h2 className="t-clue font-display uppercase tracking-wide text-moon">{t("Whose answer doesn't fit?")}</h2>
            <Answers state={state} room={room} />
            <p className="font-display text-[clamp(0.9rem,1.5vw,1.5rem)] uppercase tracking-[0.2em] text-moon-dim">
              {waiting.length
                ? t("Vote on your phone · waiting on {names}", { names: waiting.map((p) => p.name).join("، ") })
                : t("Counting…")}
            </p>
          </>
        )}
        ...
        {state.phase === "reveal" && (
          <>
            ...
            >
              {state.caught
                ? t("Caught — it was {who}", { who: `${emoji(state.oddId ?? "")} ${name(state.oddId)}` })
                : t("{who} got away with it", { who: `${emoji(state.oddId ?? "")} ${name(state.oddId)}` })}
            </motion.p>
            <div className="grid w-full max-w-5xl gap-[1.5vmin] sm:grid-cols-2">
              <Question label={t("Everyone was asked")} text={pair?.question ?? ""} />
