  ...
  return (
    <main className="flex min-h-dvh lg:h-dvh gap-[2vmin] p-[2vmin] pb-16 lg:pb-[1.6vmin]">
      ...
      <section className="flex min-w-0 flex-1 flex-col items-center justify-center gap-[1.5vmin]">
        <SketchCanvas strokes={state.strokes} live={state.live} className={state.phase === "reveal" ? "h-full max-h-[62vh] w-auto" : "h-full max-h-[78vh] w-auto"} />
        {state.phase === "reveal" && (
          <motion.p initial={{ scale: 0.94 }} animate={{ scale: 1 }} className="accent-text t-answer font-display font-bold uppercase">
            {pair?.word}
          </motion.p>
        )}
      </section>
      ...
      <aside className="flex w-[28vw] min-w-[260px] shrink-0 flex-col gap-[1.5vmin]">
        ...
        <div>
          <p className="font-display text-xs uppercase tracking-[0.25em] text-moon-deep">Category</p>
        ...
        {state.phase === "draw" && (
          <p className="font-display text-[clamp(1rem,1.6vw,1.6rem)] uppercase tracking-wide text-moon">
            ...
            <span className="ml-2 text-moon-deep">· line {Math.min(state.turn + 1, strokeTotal(state))} of {strokeTotal(state)}</span>
          </p>
        )}
        {state.phase === "vote" && (
          <p className="font-display text-[clamp(1.1rem,1.8vw,1.8rem)] uppercase tracking-wide text-moon">
            Who was faking it?
            <span className="ml-2 text-moon-deep">{votesIn}/{players.length} voted</span>
          </p>
        )}
        {state.phase === "guess" && (
          <p className="font-display text-[clamp(1.1rem,1.8vw,1.8rem)] uppercase tracking-wide text-moon">
        ...
        {state.phase === "reveal" && (
          <p className="font-display text-[clamp(1.1rem,1.8vw,1.8rem)] uppercase tracking-wide text-moon">
            ...
            <span className="block text-moon-deep">
              {state.caught === false
          ...
          {state.order.map((id) => {
            ...
            return (
              ...
              >
                ...
                {state.phase === "vote" && <span className="text-xs text-moon-deep">{voted ? "✓" : "…"}</span>}
              </div>
            );
          })}
        </div>
        ...
        {(state.phase === "draw" || state.phase === "vote" || state.phase === "guess") && (
          <button onClick={onForce} className="btn-ghost w-full py-3 text-sm">
