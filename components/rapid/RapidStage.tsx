    ...
    <div className="flex h-full flex-col items-center justify-center gap-[3vmin] text-center">
      ...
      <AnimatePresence mode="wait">
        {state.phase === "bidding" && (
          ...
          >
            <p className="font-display text-[clamp(0.9rem,1.5vw,1.6rem)] uppercase tracking-[0.25em] text-moon-dim">{t("Bid against each other — how many can you name?")}</p>
            ...
            <p className="max-w-[60ch] text-[clamp(0.75rem,1.1vw,1rem)] text-moon-dim">{t("Whoever bids highest plays the category alone. Reach the number and you win the category; fall short and the other side takes it. One point either way.")}</p>
          </motion.div>
        )}
          ...
          >
            <p className="font-display text-[clamp(0.9rem,1.5vw,1.6rem)] uppercase tracking-[0.25em] text-moon-dim">
              {state.mode === "categories"
                ? t("{team} — name {n} or more", { team: team?.name ?? "", n: state.bid })
                : RAPID_RULE[state.mode]}
            </p>
            <button onClick={onGo} className="btn-accent px-16 py-5 text-2xl">{t("Start the clock")}</button>
          </motion.div>
        )}
        ...
        {state.phase === "running" && (
          ...
          >
            ...
            {/*
              ...
              */}
            <button onClick={onTimeUp} className="btn-ghost px-10 py-3 text-lg">{t("Finish now")}</button>
          </motion.div>
        )}
        ...
        {state.phase === "judge" && (
          ...
          >
            <p className="font-display text-[clamp(1.2rem,3vw,3rem)] uppercase tracking-[0.2em] text-rose-400">{t("Time")}</p>
            ...
            {state.mode === "categories" ? (
              <>
                ...
                >
                  {count >= state.bid
                    ? t("Made it — {count} to {team}", { count, team: team?.name ?? "" })
                    : t("Short — {n} to {team}", { n: state.bid, team: state.teams[(state.turn + 1) % state.teams.length]?.name ?? "" })}
                </button>
              </>
            ) : (
              <>
                ...
                <div className="flex gap-4">
                  <button onClick={() => onScore(1)} className="btn-good px-12 py-5 text-2xl">{t("✓ Got it")}</button>
                  <button onClick={() => onScore(0)} className="btn-bad px-12 py-5 text-2xl">{t("✗ Missed")}</button>
                </div>
              </>
            )}
          </motion.div>
