...
function FaceOffStage() {
  ...
  if (generating) {
    return (
      <Generating
        title={t("Writing the survey")}
        items={generating.themes.length ? generating.themes : [t("Face-Off")]}
        note={t("A question and its top answers for every round, ranked the way a hundred people would have.")}
        onCancel={() => {
          abort.current?.abort();
          setGenerating(null);
        }}
          ...
          >
            {state.phase === "face-off" && (
              <div className="flex h-full flex-col items-center justify-center gap-[3vmin] text-center">
                ...
                <p className="t-clue max-w-[80vw] text-balance font-display uppercase tracking-wide text-moon">{t("Who takes the board?")}</p>
            ...
            {state.phase === "winner" && (
              <div className="flex h-full flex-col items-center justify-center gap-[3vmin] text-center">
                <p className="t-label font-display uppercase text-moon-deep">
                  {feudWinners(state.teams).length > 1 ? t("It's a tie") : t("Champions")}
                </p>
                <WinnerMoment>
